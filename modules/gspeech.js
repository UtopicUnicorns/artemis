(function () {
  ...
  function fullText(timedTranscript) {
    ...
    for (var i = 0; i < timedTranscript.length; i++) {
      if (timedTranscript[i]) full += timedTranscript[i].text + " ";
    }
    return full;
  }
  ...
  gspeech.recognize = function (options, callback) {
    function getTranscriptFromServer(params, onfinish) {
      ...
      if (!params.file) {
        onfinish(
          new Error(
            "No file is specified. Please specify a file path through params.file"
          )
        );
      }
      ...
      var getReq = request.get(downstreamOpts, function (error, res, body) {
        ...
        try {
          ...
          if (params.retries < maxRetries) {
            ...
            onfinish(
              new Error(
                "Could not get valid response from Google servers " +
                  "for segment starting at second " +
                  params.start
              )
            );
          }
    ...
    ffmpeg.ffprobe(file, function (err, info) {
      ...
      if (segments) {
        for (var i = 0; i < segments.length; i++) {
          var duration =
            i == segments.length - 1
              ? totalDuration - segments[i]
              : segments[i + 1] - segments[i];
          if (duration < 0) {
            callback(
              new Error(
                "segments must be a sorted array of start times, \
